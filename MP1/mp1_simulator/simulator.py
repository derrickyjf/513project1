...
def create_ado_sawtooth(max_timesteps: int, dt: float) -> np.ndarray:
    t = np.arange(0, max_timesteps, step=dt)
    # Time between trough and peak
    time_period = 10.0  # s
    # Frequency
    freq = 1 / time_period
    # We want to offset by 0.5 of a time period so that we start at 0
    offset = time_period / 2
    f = signal.sawtooth(freq * 2 * np.pi * (t + offset))
...
class Simulator:
    ...
    def __init__(self, **config):
        ...
        self.ado_acc_control_signal = create_ado_sawtooth(
            self.config["max_timesteps"], self.dt
        )
